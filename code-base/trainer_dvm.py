...
class Trainer(object):
    ...
    def train(self):
        ...
        for step in range(start, self.train_config.total_step + 1):
            ...
            image = torch.cat([image_dict['image0'], image_dict['image1'], image_dict['image2']], dim=0)
            alpha = torch.cat([image_dict['alpha0'], image_dict['alpha1'], image_dict['alpha2']], dim=0)
            trimap = torch.cat([image_dict['trimap0'], image_dict['trimap1'], image_dict['trimap2']], dim=0)
            mask = torch.cat([image_dict['mask0'], image_dict['mask1'], image_dict['mask2']], dim=0)
            fg = torch.cat([image_dict['fg0'], image_dict['fg1'], image_dict['fg2']], dim=0)
            bg = torch.cat([image_dict['bg0'], image_dict['bg1'], image_dict['bg2']], dim=0)
            ...
            fg_norm, bg_norm = fg.cuda(), bg.cuda()
            ...
            if self.train_config.val_step > 0:
                if ((step % self.train_config.val_step) == 0 or
                        step == self.train_config.total_step):  # and step > start:
